...
void loop() {
  ...
  //take in sensors
  int sens_1 = digitalRead(IR_1); //front
  int sens_2 = digitalRead(IR_2); //left
  int sens_3 = digitalRead(IR_3); //right
  ...
  if(loop_enc_count >= total_enc_dist_needed){
    go(LEFT, 0);
    go(RIGHT, 0);
    delay(5000);
  }

  if (digitalRead(sens_1) == HIGH){
    go(LEFT, 0);
    go(RIGHT, 0);
    delay(5000);
  }
...
/*
  ...
  //uturn right
  if((enc_count == total_enc_dist_needed) && last_turn == 0){
    UTURN_RIGHT();
    //reset enoder counts
    enc_count = 0;
    last_turn = 1;
  ...
  //uturn left
  if((enc_count == total_enc_dist_needed) && last_turn == 1){
    UTURN_LEFT();
    //reset enoder counts
    enc_count = 0;
    last_turn = 0;
